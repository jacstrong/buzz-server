...
/**
 * @api {post} api/admin/user/password/forgotpassword/
 * @apiName ForgotPasswordRequest
 * @apiGroup AdminPassword
 *
...
router.post('/forgotpassword', auth.optional, (req, res, next) => {
  ...
  AdminSchema.findOne({ email: reqEmail }, (err, doc) => {
    ...
    email.forgotPassword({ firstName: doc.firstName, confirmationID: passReq._id, email: reqEmail});
...
/**
 * @api {post} api/admin/user/password/forgotpassword/:id
...
router.post('/forgotpassword/:id', auth.optional, (req, res, next) => {
  ...
  PasswordConfSchema.findByIdAndRemove(id, (err, doc) => {
    ...
    AdminSchema.findById(doc.userID, (err, doc) => {
      if (err) return res.status(500).send();
      if (doc === null) return res.status(422).json({ error: { message: 'password reset request could not be found' }});
